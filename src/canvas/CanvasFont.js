...
export default class CanvasFont extends CanvasView{
    ...
    drawLabel(data, x, y, width, height, h){
        let alignX, alignY;
        let {align, font : {size, color}, decorator} = this.data
    ...
    drawFont(){
        let {font : {size, color}, spacing, fontStyle : style = [], decorator} = this.data
    ...
    getRows(data){
        ...
        let {font : {size}, spacing} = this.data
        ...
        let k = size, length = data.length, index = 0, result = []
