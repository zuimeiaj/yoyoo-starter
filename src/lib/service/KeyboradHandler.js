...
export const CombomKeysArray = Object.keys(CombomKeys).sort((a, b)=>b - a).map(item=>+item)
// 可配置的组合快捷键
const ActionKeyMaps = {
...
export const getFormatShortcutsWithAction = (key)=>{
    ...
    let combo = +arr[0]
    let k = arr[1]
    return CombomKeysArray.filter(item=>(combo & item) == item).map(item=>getKeyCodeFormat(CombomKeys[item])).join('') + '' + getKeyCodeFormat(KeyCode[k])
}
//  组合键配置 为 {actino:[0,1]}
