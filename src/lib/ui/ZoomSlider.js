...
export default class ZoomSlider extends React.Component {
    ...
    render(){
        return (<div className={'aj-zoom-slider'}>
            <div onClick={this.handleReset} ref={'value'} className={'aj-zoom_value aj-zoom_btn'}> 999</div>
            <div onClick={() => this.handleClick(context_zoom_out)} className={'aj-zoom_min aj-zoom_btn'}> -</div>
            <div ref={'track'} className={'aj-zoom-wrapper'}>
                <div ref={'slider'} className={'aj-zoom_slider'}></div>
                <div className={'aj-zoom_track'}></div>
            </div>
            <div onClick={() => this.handleClick(context_zoom_in)} className={'aj-zoom_max aj-zoom_btn'}> +</div>
        </div>)
    }
}