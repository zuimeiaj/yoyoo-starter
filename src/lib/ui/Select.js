    ...
    render(){
        ...
        return (
            <div ref={'wrapper'} style={style}
                 className={`${className} aj-component-select ${this.state.value ? 'has-value' : ''}`}>
                <input readOnly disabled={this.state.disabled} value={this.state.label} onBlur={this.onBlur}
                       onFocus={this.onFocus} className={'aj-input'}/>
                
                <i className={this.state.visible ? 'arrow arrow-down' : 'arrow'}>▴</i>
                <Icon onClick={this.close} type={'guanbi'}/>
                <div ref={'poplist'} style={{display : this.state.visible ? 'block' : 'none', top : this.state.top}}
                     className={'aj-select-poplist'}>
                    
                    <ul className={'aj-select-scroll'}>
                        {
                            this.state.options.map((item) =>{
                                let t = isBasicType(item), key, label;
                                if (t) {
                                    label = item
                                    key = item
                                } else {
                                ...
                                return <li className={this.state.value === key ? 'checked' : ''}
                                           onMouseDown={() => this.onClick(key, item, label)}
                                           key={key}>{renderItem ? renderItem(item) : label}</li>
                            })
                        }
