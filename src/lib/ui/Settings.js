...
import { component_active, component_properties_change, component_settings_lock, component_settings_show, outline_coverage_name_change } from '../util/actions';
  ...
  render() {
    return (
      <InspectorCard
        title={
          <Fragment>
            <Icon type={'shezhi1'} /> 设置
          </Fragment>
        }
        className='component-settings'
      >
        <div className={'settings_name'}>
          <EditableLabel value={this.state.value} onChange={(v) => this.onChange('alias', v)} className={'component-alias'} />
          <ToggleIcon onChange={(v) => this.onChange('settings', 'isHide', !v)} ref={'show'} selected={'eye2'} unselected={'uneye'} />
          <ToggleIcon onChange={(v) => this.onChange('settings', 'isLock', v)} ref={'lock'} selected={'lock'} unselected={'pic-unlock'} />
        </div>

        <div className={'fixation'}>
          固定
          <Checkbox onChange={(v) => this.onChange('settings', 'fixation', v)} ref={'fixation'} />
        </div>
        <div ref={'scroll'} className={'settings_overflow'}>
          <label>滚动</label> <Select onChange={this.handleOverflowChange} options={Overflow} />
        </div>
      </InspectorCard>
    );
  }
