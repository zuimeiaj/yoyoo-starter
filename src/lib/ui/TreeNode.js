...
class EditableName extends React.Component {
    ...
    handleDragOver = (e) =>{
        ...
        e.stopPropagation()
        if (checkCanDrop(e)) {
            jQuery(e.target).parent().addClass('drag-over')
        }
    }
    ...
    handleDrop = (e) =>{
        ...
        jQuery(e.target).parent().removeClass('drag-over')
    }
        ...
        return (
            <div onClick={this.handleSelect}
                ...
                <span className={'tree-node-actions'}>{
                    renderActions({id, name, path, type})
                }</span>}
            </div>
        )
    }
                ...
                return (
                    <div data-uid={item.id} key={item.id}>
                        ...
                        {item.items && item.items.length > 0 && (
                            <div className={'tree-node-has-sub'}>
                                {loop(item.items, _subpath, paddingLeft + 15, item.id)}
                            </div>
                        )}
                    </div>
                )
