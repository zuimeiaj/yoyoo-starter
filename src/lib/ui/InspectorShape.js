...
export default class InspectorShape extends React.PureComponent {
    ...
    onActive = (target) =>{
        ...
        let {
            borderWidth, borderColor, borderType, bg, shadowType, shadowColor,
            circleType, borderRadius, borderTopLeft, borderTopRight, borderBottomLeft, borderBottomRight,
            shadowBlur, shadowSpread, shadowOffsetX, shadowOffsetY
        } = this.refs
    ...
    render(){
        return (<InspectorCard title={<Fragment><Icon type={'miaobian'}/>外观</Fragment>}
                               className={'inspector-control-shape'}>
           
            <div className={'item'}><span className={'control-label'}>背景</span>
                <div className={'control-items'}><ColorInput onChange={(v) => this.onChange('background', v)}
                                                             onPreview={(v) => this.handleColorPreview('background', v)}
                                                             ref={'bg'}/></div>
            </div>
            <div className={'item'}><span className={'control-label'}>阴影</span>
                
            
            </div>
            <div className={'item'}>
                <span style={{visibility : 'hidden'}} className={'control-label'}>模糊</span>
            ...
            <div className={'item'}>
                <span style={{visibility : 'hidden'}} className={'control-label'}>圆角</span>
    ...
    render(){
        return (
            <InspectorCard title={'外观'}>
                <div className={'inspector_line-color'}><span className={'control-label'}>背景</span>
                    <div className={'control-items'}>
                        <ColorInput
                            onChange={this.handleColor}
                            onPreview={this.handleColorPreview}
                            ref={'bg'}/>
                    </div>
                </div>
            </InspectorCard>
...
class BorderAndBg extends React.Component {
    ...
    render(){
        return (
            <InspectorCard title={'外观'}>
                <div className={'inspector_line-color'}><span className={'control-label'}>边框</span>
                    <div className={'control-items'}>
                        <ColorInput
                            onChange={(v) => this.handleColor('stroke', v)}
                            onPreview={(v) => this.handleColorPreview('stroke', v)}
                            ref={'stroke'}/>
                    </div>
                </div>
                <div style={{display : this.showBg ? 'flex' : 'none'}} className={'inspector_line-color'}><span
                    className={'control-label'}>背景</span>
                    <div className={'control-items'}>
                        <ColorInput
                            onChange={(v) => this.handleColor('fill', v)}
                            onPreview={(v) => this.handleColorPreview('fill', v)}
                            ref={'bg'}/>
                    </div>
                </div>
            </InspectorCard>
