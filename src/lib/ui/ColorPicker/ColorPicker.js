...
const sbBg = require('./sb.png')
const presets = [
    'rgba(244, 67, 54,1)', 'rgba(233, 30, 99,1)', 'rgba(255, 152, 0,1)', 'rgba(33, 150, 243,1)',
    'rgba(255, 255, 255,1)', 'rgba(0, 0, 0,1)', 'rgba(139, 195, 74,1)', 'rgba(103, 58, 183,1)',
    'rgba(63, 81, 181,1)', 'rgba(158, 158, 158,1)'
]
...
export default class ColorPicker extends React.Component {
    ...
    handleSBAreaMove = ({pointX, pointY}) =>{
        ...
        let x = pointX - half, y = pointY - half
        this.sb.left(x)
        this.sb.top(y)
        // update hsv
    ...
    applyGradientColor = (color) =>{
        if (this.currentGradient) {
            ...
            let line_gradient = `linear-gradient(90deg,${list.map(item =>{
    ...
    render(){
        return (<div onClick={(e) => e.stopPropagation()} style={{display : 'none'}} ref={'picker'}
                     className={'aj-color-picker'}>
            <div className={'header-bar'}>
                <Tooltip placement={'left'} title={' 选择系统颜色拾取时，需要关闭该面板才能生效'}>
                    <span><input onBlur={this.handleSystemColorBlur} onChange={this.handleSystemColorChange} type={'color'}/></span>
                </Tooltip>
            ...
            <div className={'values'}>
                ...
                <input ref={'hex'} onChange={this.handleHexChange} data-event="ignore"
                       style={{width : '23%', paddingLeft : 2}} className={'aj-input'}></input>
            </div>
            ...
            <div className={'colors'}>
                <div>
                    {
                        presets.map(item => <a onClick={() => this.handlePresetsClick(item)} style={{background : item}}
                                               key={item}/>)
                    }
                </div>
...
class CommonColor extends React.Component {
    ...
    render(){
        return <div>
            {
                this.state.list.map(item => <a onClick={() => this.props.onClick(item)} style={{background : item}}
                                               key={item}/>)
            }
        </div>
    }
