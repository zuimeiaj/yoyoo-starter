...
export default class LineNumber extends React.Component {
    ...
    handleMove = (coords) =>{
        if (coords.isH) {
            this.line.show().left(config.editorDomRect.left + 5).top(coords.pageY + 10).text(Math.round(pointToWorkspaceCoords(coords).y))
        } else {
            this.line.show().left(coords.pageX + 5).top(config.editorDomRect.top + 10).text(Math.round(pointToWorkspaceCoords(coords).x))
        }
    }
