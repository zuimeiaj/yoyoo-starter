...
export default class InspectorAnchor extends React.PureComponent {
    ...
    render(){
        return (
            <InspectorCard title={<Fragment><Icon type={'QuXiaoGuDing'}/> 固定</Fragment>} className={'inspector-anchor'}>
                <Checkbox className={'anchor-enabled'} ref={'enabled'} onChange={(v) => this.onChange('enabled', v)} label={'启用固定'}/>
                <div className={'item'}>
                    <NumberInput ref={'top'} onChange={(v) => this.onChange('top', v)}
                                 label={<Icon type={'wenzidingbuduiqi'}/>}/>
                    <NumberInput ref={'bottom'} onChange={(v) => this.onChange('bottom', v)}
                                 label={<Icon type={'wenzidibuduiqi'}/>}/>
                </div>
                <div className={'item'}>
                    <NumberInput ref={'left'} onChange={(v) => this.onChange('left', v)}
                                 label={<Icon type={'ralign-left'}/>}/>
                    <NumberInput ref={'right'} onChange={(v) => this.onChange('right', v)}
                                 label={<Icon type={'ralign-right'}/>}/>
                </div>
            </InspectorCard>)
    }
}