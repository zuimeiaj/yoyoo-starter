...
export default class TextInput extends React.Component {
    ...
    render(){
        ...
        return (<div ref={"g"} style={style} className={`aj-number-input-control ${className}`}>
            {label && <label className={'aj-number-label'}>{label}</label>}
            <span className={'aj-number-input-wrapper'}>
                <input style={{color:'#666'}} onKeyUp={this.handleKeyUp} placeholder={placeholder} className={'aj-number-input'}
                       onInput={this.onInput} ref={'input'} data-event="ignore"/>
            </span>
        </div>)
    }
}