...
export default class ViewCircle extends ViewController {
    ...
    componentDidMount(){
        ...
        let {border : {width : strokeWidth}, circle : {array, offset}} = this.properties
        ...
        new Draggable(center, {
            ...
            onDragMove : ({deltaX}) =>{
                ...
                let line = this.refs.line, d = 2 * r * Math.PI
                ...
                Event.dispatch(component_properties_change, {
                    ...
                    key : ['circle', 'border'],
                    value : [
                        Object.assign({}, this.properties.circle, {offset, array}),
                        Object.assign({}, this.properties.border, {width : strokeWidth})
                    ]
                })
        ...
        let cx, cy, rotation = this.dashAngle, currentAngle = 0, angle = 0
        new Draggable(dash, {
            onDragStart : (_, e) =>{
                let {width, height} = this.properties.transform
    ...
    renderContent(){
        let {circle : {array, offset}, border : {width : sw, color}, transform : {width, height}} = this.properties
        let {x, y} = this._getDasharrayPosition(360-this.dashAngle, (width - sw * 2) / 2 - 10)
        return (
            <svg style={{width : '100%', height : '100%'}} xmlns={'http://www.w3.org/2000/svg'}
                 data-uid={this.properties.id}
                 className={'view-bubble'}>
                <circle strokeDasharray={array}
                        ...
                        strokeWidth={sw}/>
                <circle ref={'dash'} className={'component-control-dot'} r={4} cx={x}
                        cy={y}/>
