...
export default class HighlightComponentOnComponentEneter extends NoZoomTransform {
    ...
    applyToDom(){
        ...
        Dom.of(t).left(x).top(y).width(width)
        Dom.of(l).left(x).top(y).height(height)
        Dom.of(r).left(x + width).top(y).height(height + 1)
        Dom.of(b).left(x).width(width).top(y + height)
