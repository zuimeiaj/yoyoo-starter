...
export default class ViewLine extends ViewController {
    ...
    render(){
        let {transform : {x, y, rotation, width, height}, border : {color, style}} = this.properties
        let strokeDash = {}
        if (style == 'dashed') {
        ...
        return (
            <div data-uid={this.properties.id}
                 style={{
                     width, height,
                     left : x, top : y, transform : `rotate(${rotation}deg)`
                 }} className={'view-line'} ref={'container'}>
