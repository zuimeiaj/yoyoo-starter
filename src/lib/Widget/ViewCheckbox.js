 ...
 *  多选组件：渲染选项列表（方框 + 文本），行首 ">" 的选项为默认选中态（可多个）。
...
export default class ViewCheckbox extends ViewController {
  ...
  onDBClick(e) {
    if (getGroupId()[this.properties.id] && getTemporaryGroup().isLockChildren) {
      ...
      poplist.value = this.properties.checkboxOptions || '';
      Dom.of(poplist)
        .show()
  ...
  getWrapperClassName() {
    return super.getWrapperClassName() + ' view-checkbox';
  }

  _handlePoplistChange = (e) => {
    this.properties.checkboxOptions = e.target.value;
    this.forceUpdate();
  ...
  renderContent() {
    let options = (this.properties.checkboxOptions || '').split('\n').filter((t) => t.trim());
    let horizontal = this.properties.direction === 'horizontal';
    return (
      <Fragment>
        <div ref={'list'} className={`view-checkbox-list ${horizontal ? 'horizontal' : ''}`}>
          {options.map((opt, i) => {
            let text = opt.trim();
            let checked = text.startsWith('>');
            if (checked) text = text.slice(1);
            return (
              <div className={'view-checkbox-item'} key={i}>
                <span className={`view-checkbox-box ${checked ? 'checked' : ''}`} />
                <span className={'view-checkbox-label'} style={{ color: this.properties.font.color }}>
                  {text}
                </span>
              </div>
            );
        ...
        <textarea
          ...
          placeholder={'换行分隔，">" 表示默认选中该项，可多个，示例：\n > a \n > b \n c '}
        ></textarea>
      </Fragment>
    );
  }
