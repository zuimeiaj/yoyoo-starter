...
export default class ViewPolygon extends ViewController {
    ...
    _getPath = (left) =>{
        ...
        const y = 2
        return `M12 ${y} L${width - 12} ${y} Q${width - 2} ${y} ${width - 2} 10 L${width - 2} ${height - 20} Q${width - 2} ${height - 10} ${width - 12} ${height - 10} L${left + 20} ${height - 10} L${left + 10} ${height} L${left} ${height - 10} L10 ${height - 10} Q2 ${height - 10} 2 ${height - 20}  L2 12 Q2 2 12 2`
    }
    ...
    renderContent(){
        ...
        let {border : {width : sw, color, style}, bg, bubble : {left}} = this.properties
        let triangleSize = 10
        let strokeDash = {}
        if (style == 'dashed') {
        ...
        return (
            <svg style={{width : '100%', height : '100%'}} xmlns={'http://www.w3.org/2000/svg'}
                ...
                <path ref={'line'}
                      d={this._getPath(left)}
                      {...strokeDash}
                      style={{stroke : color, fill : bg}}
                      strokeWidth={sw}/>
