...
export default class ViewText extends ViewController{
    ...
    initProperties(){
        ...
        let {font : {size, color}, fontStyle, decorator, spacing, align} = this.properties
        if(spacing) {
    ...
    renderContent(){
        return (<div ref={'wrapper'} className={`view-text view-text_${this.properties.type}`}>
            <div onPaste={this.handlePaste} onKeyUp={this._handleKeyUp}
                 data-event="ignore" ref={'text'}/>
        </div>)
    }
}
