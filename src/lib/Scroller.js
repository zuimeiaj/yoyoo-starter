...
import {getScreeTransform, workspaceToPointCoords} from "./global";
...
export default class Scroller extends React.Component{
    ...
    scrollToCenter = (scale, e = {pageX : window.innerWidth / 2, pageY : window.innerHeight / 2})=>{
        ...
        if(this._lastMouseX === null) this._lastMouseX = e.pageX
        if(this._lastMouseY === null) this._lastMouseY = e.pageY
        let detalMouseX = e.pageX - this._lastMouseX
        let detalMouseY = e.pageY - this._lastMouseY
        this._lastMouseX = e.pageX
        this._lastMouseY = e.pageY
        if(this.type === 'h' && Math.abs(detalMouseX) > 0) {
            this._pointOffsetX = null
        }
        if(this.type === 'v' && Math.abs(detalMouseY) > 0) {
            this._pointOffsetY = null
        }
        if(this.type === 'h' && this._pointOffsetX === null) {
            this._pointOffsetX = (e.pageX - workspaceToPointCoords().x) / scale
        } else if(this.type === 'v' && this._pointOffsetY === null) {
    ...
    render(){
        ...
        return (<div className={`aj-scroller-${type}`}>
