...
const batchDeletePage = (ids) =>{
    ...
    setPageData(pages)
    realIds = realIds.filter(item => item)
    return realIds.length > 0 ? deletePageApi(realIds.join(',')) : Promise.resolve()
...
export const deletePage = (id) =>{
    return new Promise((resolve) =>{
        ...
        if (item._id) { // 删除服务端数据
            deletePageApi(item._id).then(resolve)
        } else {
            resolve()
        }
...
export async function saveToRemoteFromStorage(pages){
    try {
        //已有项目和页面，只保存当前操作的页面就行了
        if (window.GlobalSaving) {
            return
        }
        if (pages.length > 0) {
            let p = getQuery().p;
            Event.dispatch(context_save_start)
            for (let i = 0; i < pages.length; i += 1) {
                let page = pages[i]
                if (page._id && page.projectid == p) { // update
                    await savePage(page)
                }
            }
            clearPageStorage()
        ...
        console.error(e)
        Event.dispatch(context_save_failed)
    }
}