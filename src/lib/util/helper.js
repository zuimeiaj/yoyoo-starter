...
export const uuid = (prefix = '')=>{
    return (prefix + Math.random().toString().slice(2) + Math.random().toString().slice(2)).slice(0, 16)
}
...
export const ANIMATIONS = [
    ...
    },
    {
        "label" : "弹入",
        "key" : "bounceIn"
    }, {"label" : "下弹入", "key" : "bounceInDown"}, {"label" : "左弹入", "key" : "bounceInLeft"}, {
        "label" : "右弹入",
        "key" : "bounceInRight"
    }, {"label" : "上弹入", "key" : "bounceInUp"}, {"label" : "弹出", "key" : "bounceOut"}, {
        "label" : "下弹出",
        "key" : "bounceOutDown"
    }, {"label" : "左弹出", "key" : "bounceOutLeft"}, {"label" : "右弹出", "key" : "bounceOutRight"}, {
        "label" : "上弹出",
        "key" : "bounceOutUp"
    }, {"label" : "淡入", "key" : "fadeIn"}, {"label" : "下淡入", "key" : "fadeInDown"}, {
        "label" : "下淡入",
        "key" : "fadeInDownBig"
    }, {"label" : "左淡入", "key" : "fadeInLeft"}, {"label" : "左淡入2", "key" : "fadeInLeftBig"}, {
        "label" : "右淡入",
        "key" : "fadeInRight"
    }, {"label" : "右淡入2", "key" : "fadeInRightBig"}, {"label" : "上淡入", "key" : "fadeInUp"}, {
        "label" : "上淡入2",
        "key" : "fadeInUpBig"
    }, {"label" : "淡出", "key" : "fadeOut"}, {"label" : "下淡出", "key" : "fadeOutDown"}, {
        "label" : "下淡出2",
        "key" : "fadeOutDownBig"
    }, {"label" : "左淡出", "key" : "fadeOutLeft"}, {"label" : "左淡出2", "key" : "fadeOutLeftBig"}, {
        "label" : "又淡出",
        "key" : "fadeOutRight"
    }, {"label" : "右淡出2", "key" : "fadeOutRightBig"}, {"label" : "上淡出", "key" : "fadeOutUp"}, {
        "label" : "上淡出2",
        "key" : "fadeOutUpBig"
    }, {"label" : "翻转", "key" : "flip"}, {"label" : "垂直转入", "key" : "flipInX"}, {"label" : "水平转入", "key" : "flipInY"}, {
        "label" : "垂直转出",
        "key" : "flipOutX"
    }, {"label" : "水平转出", "key" : "flipOutY"}, {"label" : "动感", "key" : "lightSpeedIn"}, {
        "label" : "动感出",
        "key" : "lightSpeedOut"
    }, {"label" : "旋入", "key" : "rotateIn"}, {"label" : "左下旋入", "key" : "rotateInDownLeft"}, {
        "label" : "右下旋入",
        "key" : "rotateInDownRight"
    }, {"label" : "左上旋入", "key" : "rotateInUpLeft"}, {"label" : "右上旋入", "key" : "rotateInUpRight"}, {
        "label" : "旋出",
        "key" : "rotateOut"
    }, {"label" : "左下旋出", "key" : "rotateOutDownLeft"}, {"label" : "右下旋出", "key" : "rotateOutDownRight"}, {
        "label" : "左上旋出",
        "key" : "rotateOutUpLeft"
    }, {"label" : "右上旋出", "key" : "rotateOutUpRight"}, {"label" : "上滑入", "key" : "slideInUp"}, {
        "label" : "下滑入",
        "key" : "slideInDown"
    }, {"label" : "左滑入", "key" : "slideInLeft"}, {"label" : "右滑入", "key" : "slideInRight"}, {
        "label" : "滑出",
        "key" : "slideOutUp"
    }, {"label" : "下滑出", "key" : "slideOutDown"}, {"label" : "左滑出", "key" : "slideOutLeft"}, {
        "label" : "右滑出",
        "key" : "slideOutRight"
    }, {"label" : "放大", "key" : "zoomIn"}, {"label" : "下放大", "key" : "zoomInDown"}, {
        "label" : "左放大",
        "key" : "zoomInLeft"
    }, {"label" : "右放大", "key" : "zoomInRight"}, {"label" : "上放大", "key" : "zoomInUp"}, {
        "label" : "缩小",
        "key" : "zoomOut"
    }, {"label" : "下缩小", "key" : "zoomOutDown"}, {"label" : "左缩小", "key" : "zoomOutLeft"}, {
        "label" : "右缩小",
        "key" : "zoomOutRight"
    }, {"label" : "上缩小", "key" : "zoomOutUp"}, {"label" : "掉落", "key" : "hinge"}, {
        "label" : "欧耶",
        "key" : "jackInTheBox"
    }, {"label" : "转入", "key" : "rollIn"}, {"label" : "转出", "key" : "rollOut"}
]
export const getDropView = (e, data)=>{
    ...
    if(data.type === 'AdvanceComponent') {
        ...
        let send = {
            ...
            id : uuid('sb_'),
            transform : {
                x, y, width : data.width || 100, height : data.height || 100, rotation : 0
            }
        }
...
export const activeAfterMounting = (target)=>{
    setTimeout(()=>{
        if(Array.isArray(target)) {
            Event.dispatch(selection_group, target.map(item=>item.view))
        } else {
...
export const randomId = (prefix = '')=>{
    return prefix + '_' + Math.random().toString().slice(2)
}
...
export const getClientBoundingRect = ({x, y, width, height, rotation})=>{
    ...
    let xarr = []
    let yarr = []
    let points = deg.dot(rect.T()).T().valueOf().map(item=>{
        let x1 = item[0] + wc + x
        let y1 = -(item[1] - hc) + y
        xarr.push(x1)
        yarr.push(y1)
