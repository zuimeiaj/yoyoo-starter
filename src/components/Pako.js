...
class PAKOJS extends React.Component {
    ...
    render(){
        return (<Form style={{padding : 25}} onSubmit={this.handleSubmit}>
            <div>Input</div>
            {
                this.props.form.getFieldDecorator('input')(<Input.TextArea rows={5}
                                                                           placeholder={'encode after string'}></Input.TextArea>)
            }
            <div>Output</div>
            {
                this.props.form.getFieldDecorator('output')(<Input.TextArea rows={5}
                                                                            placeholder={'decode after string'}></Input.TextArea>)
            }
            <div></div>
            <Button type={'primary'} htmlType={'submit'}>Inflate</Button>
        </Form>)
    }
}
