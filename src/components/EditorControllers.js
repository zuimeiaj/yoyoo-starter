...
class EditorControllers extends React.Component{
    ...
    _initializingProject = ()=>{
        let query = getQuery()
        if(query.p) {
            fetchProjectInfo(query.p).then((res)=>{
                setStore('project', res.data.info)
                setStore('pages', res.data.pages)
                initPageSizeWithProjectType(res.data.info)
                fetchMaster().then(()=>{
                    Event.dispatch(project_initialized, res.data)
                    Event.dispatch(refresh_project_name, res.data.info.name)
                    if(getQuery().m) {
                        Event.dispatch(context_mode_change, 'MASTER')
                    } else {
                        Event.dispatch(pages_load_end, res.data.pages)
                    }
                })
            }).catch(()=>{
    ...
    componentWillMount(){
        ...
        let projectPages = pages.filter(item=>item.projectid == query.p)
        if(projectPages.length > 0) {
            let hide = message.success('数据同步中...', 0)
            saveToRemoteFromStorage(projectPages).finally(()=>{
                hide()
                clearPageStorage()
                this._initializingProject()
            })
        } else {
            clearPageStorage()
            this._initializingProject()
    ...
    handleEditorModeChange = (type, masterId)=>{
        ...
        if(type == 'PROJECT') {
            ...
            setTimeout(()=>{
                if(data.length > 0) {
                    Event.dispatch(selection_group, data.map(item=>item.view))
                } else {
    ...
    handleApplyUngroup = (block)=>{
        ...
        setTimeout(()=>{
            ...
            else Event.dispatch(selection_group, items.map(item=>item.view))
        })
    }
    ...
    handlePasteToMouse = (target, event)=>{
        ...
        setTimeout(()=>{
            if(data.length > 1) {
                Event.dispatch(selection_group, views.map(item=>item.view))
            } else {
...
// BG WIDTH GUIDES
    ...
    handleSaveWorkspace = async()=>{
        ...
        if(!window.NetworkConnected) return
        if(getQuery().m) {
            let nodes = getMasterDataFromStorage()
            if(nodes) {
                await saveMaster({nodes : nodes})
                clearCurrentMasterFromStorage()
            }
        } else {
            // 是否有本地缓存
    ...
    _coverageHanlder = (level, zIndex)=>{
        ...
        let stateItems = proxyAllPropsChange(this.state.items, view, 'zIndex',
            (item)=>typeof zIndex === 'undefined' ? item.zIndex + level : zIndex)
        this.setState({items : stateItems}, this.pushHistory)
    }
    /**
     * send to backward
     ...
     */
    coverageFront = ()=>{
        let max = Math.max.apply(null, this.state.items.map(item=>item.zIndex)) + 1
     ...
     */
    coverageBack = ()=>{
        let min = Math.min.apply(null, this.state.items.map(item=>item.zIndex)) - 1
    ...
    handlePageSelect = (pageid)=>{
        selectPage(pageid).then(async(items)=>{
            ...
            if(items.length > 0) {
                let max = Math.max.apply(null, items.map(item=>item.zIndex)) + 1
                setLastCoverageIndex(max + 1)
            } else {
...
// Duplicate,支持在block中使用
    ...
    _handleSelectionWithType = (t, screen, offset, rect)=>{
        ...
        if(config.selection == 'cross') {
            return ((x + t.width > rect.x && rect.x + rect.width > x) &&
                (y + t.height > rect.y && rect.y + rect.height > y))
        }
        return x > rect.x && x + t.width < rect.x + rect.width &&
            y > rect.y && y + t.height < rect.height + rect.y
    }
    /**
     * Ctrl Shift O
     */
...
export default withRouter(EditorControllers)