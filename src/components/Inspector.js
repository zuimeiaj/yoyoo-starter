...
class InspectorProps extends Component{
    ...
    render(){
        ...
        return <div className={'root-layout-inspector'}>
            {
                this.state.keys.map(key=>{
                    ...
                    return <Popover overlayClassName={'ins-props-overlay'} {...props}
                                    title={<div>{item[0]} {item[2] == 'controlled' &&
                                    <Icon type={'guanbi'} onClick={()=>this.handleCloseLayer(key)}/>}</div>} key={key}
                                    trigger={Trigger[key] || 'hover'}
                                    placement={'bottom'}
                                    overlayStyle={{maxWidth : 260}}
                                    content={<DynamicWrapper onChange={this.handleChange} field={key}
                                                             value={this.target.properties[key]}/>}>
                        <span className={'ins-props-item'}><Icon type={item[1]}/></span>
                    </Popover>
                })
            }
        </div>
                    ...
                    this.state.keys.map(key=>{
                        return (
                            <Collapse title={LABELS[key][0]}>
                                <DynamicWrapper onChange={this.handleChange}
                                                field={key}
                                                value={this.target.properties[key]}/>
                            </Collapse>
                        )
