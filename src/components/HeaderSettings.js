...
import {
    ...
    CombomKeysArray,
    updateKeyCodeMap
} from "../lib/service/KeyboradHandler";
...
class ShortcutsItem extends React.PureComponent{
    ...
    render(){
        ...
        return <div className={'settings_shortcut-item'}>
            <span className={'settings_shortcuts-name'}>{ActionNames[UpdateActionHandlers[item]]}</span>
            {
                editMode ? CombomKeysArray.map(item=>
                        <span key={item}
                              onClick={()=>this.handleComboClick(item, comboKeys)}
                              className={`settings_shortcuts-combo settings_shortcuts-editmode ${comboKeys.indexOf(item) > -1 ? 'selected' : ''}`}>{getKeyCodeFormat(CombomKeys[item])}</span>) :
                    comboKeys.map(item=>
                        <span key={item}
                              className={'settings_shortcuts-combo'}>{getKeyCodeFormat(CombomKeys[item])}</span>)
            }
            {editMode ? <input onKeyUp={this.handleChange} data-event="ignore" value={getKeyCodeFormat(KeyCode[code])}
                               className={'settings_shortcuts-key'}/> :
...
class Shortcuts extends React.Component{
    state = {
        mode : false,
        actions : Object.keys(ActionKeyCodeMaps).sort((a, b)=>a.charCodeAt(0) - b.charCodeAt(0)).map(item=>ActionKeyCodeMaps[item])
    }
    ...
    render(){
        return (
            <div className={'header_shortcuts'}>
                <div className={'header_shortcuts-custom'}><a
                    onClick={this.toggleMode}>{this.state.mode ? '保存' : '自定义快捷键'} </a>
                </div>
                
                {
                    this.state.actions.map(item=>{
                        return <ShortcutsItem onComboChange={this.handleComboChange}
                                              onChange={this.handleChange}
                                              key={item}
                                              mode={this.state.mode} item={item}/>
                    })
                }
            </div>
        )
...
export class Settings extends React.Component{
    ...
    componentWillMount(){
        fetchUserInfo().then((res)=>{
            let data = res.data
            if(data.shortcuts) {
                updateKeyCodeMap(data.shortcuts)
            }
            if(data.preference) {
                updatePreferences(data.preference, false)
            }
        })
        ...
        return (
            <div ref={'g'} className={'header_settings-panel'}>
                ...
                <div className={'settings-panel_bar'}>
                    <div>
                        <Button onClick={this.hide} type={'default'}>取消</Button> <Button onClick={this.save}
...
export default class HeaderSettings extends React.Component{
    ...
    render(){
        return (
            <React.Fragment>
                <IconText onClick={this.handleSettingClick} className={'header_action-item'}
                          icon={'shezhi1'}>设置</IconText>
                
                <a href={COMMUNITY_TUTORIAL}>
                    <IconText className={'header_action-item'} icon={'bangzhujiaocheng'}>教程</IconText>
                </a>
                <a href={COMMUNITY_URL}>
                    <IconText className={'header_action-item'} icon={'shequ'}>社区</IconText>
                </a>
            </React.Fragment>
        )
    }
