...
class OutlinePrefabs extends React.Component{
    ...
    render(){
        return (<div className="root-layout-side-assets root-layout-side-master">
            ...
            {this.state.template.length === 0 && <div className={'side-prefabs_tips'}>
                <Icon type={'jianyi'}/>
                <div className={'side-prefabs_tips-text'}>在工作面板中选中组件，点击右键菜单，设为为母版。
                    <br/>注意：母版的应用范围仅限于项目中，当母版被修改后，项目所有使用该母版的页面都会自动同步更新
                </div>
            </div>}
        </div>)
    }
...
export default withRouter(OutlinePrefabs)