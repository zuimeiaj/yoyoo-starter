...
export default class HistoryControl extends React.Component{
    ...
    redo = ()=>{
        if(!this.state.disableRedo)
            Event.dispatch(context_redo)
    }
    undo = ()=>{
        if(!this.state.disableUndo)
            Event.dispatch(context_undo)
    }
    
    render(){
        return (
            <Fragment>
                <IconText onClick={this.undo} icon={'undo2'}
                          className={`header_action-item ${this.state.disableUndo ? 'disabled' : ''}`}>
                    撤销
                </IconText>
                <IconText onClick={this.redo} icon={'redo'}
                          className={`header_action-item ${this.state.disableRedo ? 'disabled' : ''}`}>
                    重做
                </IconText>
            </Fragment>
        )
