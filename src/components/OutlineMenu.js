...
export default class OutlineMenu extends React.Component{
    ...
    render(){
        return (<ul className="root-layout-side-menu">
            {
                this.state.menus.map(item=>{
                    return <li onClick={()=>this.props.onChange(item.type)} key={item.icon}>
                        <IconText className={item.type === this.props.selected ? 'selected' : ''}
                                  icon={item.icon}>{item.text}</IconText>
                    </li>
                })
            }
        </ul>)
