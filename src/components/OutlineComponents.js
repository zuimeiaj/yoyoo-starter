...
export default class OutlineComponents extends CacheState{
    ...
    //        findAllLibAndConetnt().then(async(res)=>{
    //            let list = res.data.docs.map(async item=>{
    //                item.content = item.content.map(async item=>{
    //                    let data = infalte(item.data)
    //                    data.name = item.name;
    //                    data.icon = data.image
    //                    return data
    //                })
    //                item.content = await Promise.all(item.content)
    //                return item
    //            })
    //            let items = []
    //            for(let i = 0; i < list.length; i++) {
    //                let item = await list[i]
    //                items.push(item)
    ...
    //}
    render(){
        return (<div className="root-layout-side-components">
                    ...
                    BaseComponents.map(item=>{
                        return (
                            <Draggable key={item.name} params={item}>
                                <IconText className={'base-component'} icon={item.icon}>{item.name}</IconText>
                            </Draggable>)
                    })
                }
            </div>
...
class WrapperAdvs extends React.PureComponent{
    render(){
        ...
        return <Collapse collapse={true} className={'component-group-title'} title={item.name}>
...
export class BaseComponentsActionBar extends React.Component{
    ...
    render(){
        ...
        return (<div className={'base-components_action-bar'}>
            {
                BaseComponents.map(item=>{
                    return (
