...
class OutlineCoverage extends React.Component{
    ...
    render(){
                    ...
                    return <TreeNode key={item.id}
                                     title={
                                         <div className={'node-title-wrapper ' + (item.settings.isHide ? 'hide' : '')}>
                                             ...
                                             <Draggable namespace={'nodedrag'} params={item.id}><EditableLabel
                                                 onChange={(value)=>this.handleNameChange(item.id, value)}
                                                 value={item.alias}/></Draggable>
                                             {item.settings.isHide &&
                                             <Icon onClick={(e)=>this.showComponent(item.id, e)} type={'uneye'}/>}
                                         </div>}>
                        {renderTreeNode(item.items)}
                    </TreeNode>
                }
                return <TreeNode key={item.id}
                                 title={<div className={'node-title-wrapper ' + (item.settings.isHide ? 'hide' : '')}>
        ...
        return (<div ref={'coverage'} className={'outline-pages outline-converages'}>
            <div className={'action-bar'}><Search placeholder={'按回车搜索'} onSearch={this.handleSearch}/></div>
            {
...
export default withRouter(OutlineCoverage)
