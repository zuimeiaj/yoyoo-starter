    ...
    render(){
        return (
            <span className={'header_action-item'}>
                <Poplist overlay={<Menu onClick={this.handleClick} style={{width : 200}}>
                    {
                        ()=>CoverageActions.map(item=>{
                            const disabled = getFirstResponder()
                            return <MenuItem disabled={!disabled} key={item.action} action={item.action}
                                             label={item.label}
                                             extra={item.extra}/>
                        })
                    }
